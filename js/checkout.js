...
(() => {
  ...
  const renderHeader = () => {
    ...
    if (!DOM.headerActions) {
      console.warn(
        "Header actions container not found. Skipping header rendering."
      );
      return;
    }
    ...
    if (isLoggedIn) {
      ...
      headerHTML =
        `<a href="login.html" class="button-primary nav-btn">Login</a>` +
        headerHTML;
    }
    ...
    if (DOM.cartIcon) {
      ...
      console.warn(
        "cartIcon not found for event listener attachment after rendering header."
      );
    }
  ...
  const toggleCartDrawer = () => {
    ...
    console.log(
      "Cart drawer toggled. Visibility status:",
      DOM.cartDrawer?.classList.contains("visible")
    );
  };
              ...
              return `
                    <div class="cart-item">
                        <div class="cart-item-details">
                            ...
                            <p class="cart-item-sessions">${
                              item.sessionsPerWeek
                            } session(s) per week: ${sessionDetails}</p>
  ...
  const updateCartInfo = () => {
    if (!DOM.cartItemCount || !DOM.cartTotalPrice) {
      console.warn(
        "Cart item count or total price element not found. Cannot update cart info."
      );
      return;
    }
    ...
    console.log(
      `Cart info updated: ${totalItems} items, Total: ${totalPrice.toFixed(
        2
      )}RM`
    );
  };
  ...
  const saveCart = () => {
    try {
      ...
      console.log(
        "Cart saved to localStorage. Content:",
        JSON.parse(localStorage.getItem("chesserCart"))
      );
    } catch (e) {
  ...
  const renderCheckoutPage = () => {
    ...
    if (!DOM.orderSummaryItems || !DOM.orderTotalPrice || !DOM.paymentForm) {
      console.warn(
        "Checkout page elements not fully found. Cannot render checkout page summary."
      );
      return;
    }
            ...
            .map((item) => {
              ...
              return `
                    <div class="summary-item">
                        ...
                        <p class="summary-item-sessions">${
                          item.sessionsPerWeek
                        } session(s): ${sessionDetails}</p>
                    </div>`;
            })
            .join("")
  ...
  const showCustomAlert = (message) => {
    if (document.querySelector(".feedback-modal.show")) {
      console.log(
        "An alert is already visible. Not showing new alert:",
        message
      );
      return;
    }
  ...
  const initialize = () => {
    ...
    DOM.cartDrawerOverlay = getElement(
      "cart-drawer-overlay",
      "Cart Drawer Overlay"
    );
    DOM.newCloseCartBtn = getElement(
      "new-close-cart-btn",
      "New Close Cart Button"
    );
    ...
    if (document.body.classList.contains("checkout-page")) {
      DOM.orderSummaryItems = getElement(
        "order-summary-items",
        "Order Summary Items"
      );
      DOM.orderTotalPrice = getElement(
        "order-total-price",
        "Order Total Price"
      );
    ...
    if (DOM.newCloseCartBtn) {
      console.log(
        "Attempting to attach click listener to #new-close-cart-btn:",
        DOM.newCloseCartBtn
      );
      ...
      console.warn(
        "newCloseCartBtn not found. Cart close by new button may not work."
      );
    }

    if (DOM.cartDrawerOverlay) {
      console.log(
        "Attempting to attach click listener to #cart-drawer-overlay:",
        DOM.cartDrawerOverlay
      );
      ...
      console.warn(
        "cartDrawerOverlay not found. Cart overlay close may not work."
      );
    }
    ...
    if (document.body.classList.contains("checkout-page")) {
      ...
      if (DOM.paymentForm) {
        DOM.paymentForm.addEventListener("submit", (e) => {
          ...
          showCustomAlert(
            "Thank you for your purchase! Your training begins now."
          );
          cart = [];
          saveCart();
          setTimeout(() => {
        ...
        console.warn(
          "paymentForm not found for event listener on checkout page."
        );
      }
    }
    ...
    if (header) {
      ...
      const applyScrollEffect = () => {
        if (window.scrollY > 50) {
          // Adjust 50px as needed for when the effect kicks in
    ...
    console.log(
      "checkout.js initialized. window.chesserCart.add is now available."
    );
  };
...
document.getElementById("card-number").addEventListener("input", function (e) {
  ...
  let formatted = value.replace(/(.{4})/g, "$1 ").trim(); // Разбить по 4 цифры
...
document.getElementById("card-expiry").addEventListener("input", function (e) {
  ...
  if (value.length >= 3) {
    value = value.slice(0, 2) + "/" + value.slice(2); // Добавить слэш после первых двух цифр
  }
